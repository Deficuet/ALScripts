...
function var_0_0.execute(arg_1_0, arg_1_1)
	...
	local var_1_3 = getProxy(NewEducateProxy):GetCurChar()

	if var_1_3:GetResByType(NewEducateChar.RES_TYPE.REFRESH_CHOICE) <= 0 then
	...
	pg.ConnectionMgr.GetInstance():Send(29105, {
		...
		if arg_2_0.result == 0 then
			...
			getProxy(NewEducateProxy):Cost({
				...
				id = var_1_3:GetResIdByType(NewEducateChar.RES_TYPE.REFRESH_CHOICE)
			})
			arg_1_0:sendNotification(GAME.NEW_EDUCATE_REFRESH_CHOICE_DONE, {
				idx = var_1_2,
				newId = var_2_0:GetChoices()[var_1_2]
			})
		else
