return {
	fadeOut = 1.5,
	mode = 2,
	id = "JIZHANRENZHEZHICHENG16",
	placeholder = {
		"playername"
	},
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "各地で暴れ回る魑魅魍魎の対応に追われていたところ、島風から朗報がもたらされた――",
			bgm = "sk-az-pv2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "近隣のウサウサ城の城主がこちらの要請に応じ、城外の森で会合を開いてくれた。情勢は一気に好転した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "待ち合わせ場所に早めに到着すると、空気にほのかな松の香りが漂っていた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "しばらくして、思いもよらぬ形でその盟友と顔を合わせることになった……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 5,
			say = "Zzzz……Zzzz……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			withoutActorName = true,
			hideRecordIco = true,
			actor = 101512,
			nameColor = "#A9F548FF",
			say = "ウサギたちが群れを成して林間の小道をぴょんぴょんと跳ねてやって来たかと思うと、目の前で立ち止まった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			withoutActorName = true,
			hideRecordIco = true,
			actor = 101512,
			nameColor = "#A9F548FF",
			say = "その頭と思わしきウサギの背には、ぐっすりと眠っている少女がいた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 4,
			say = "ふあ……もう着いた……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			say = "高貴なるウサウサ城城主、ラフィー……盟友の召集に応じて参上……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 3,
			say = "大丈夫、イタズラする妖鬼をやっつけるの……ラフィー、手伝うから……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			say = "でも……うん……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			say = "城主指揮官、疲れてそう……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 3,
			say = "一度、ねんねしたほうがいい……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			say = "ラフィーのウサウサ城には、枕がいっぱい……布団も、いっぱいあるから……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 3,
			say = "ラフィーと……一緒にウサウサ城に帰ろ？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			portrait = "zhihuiguan",
			nameColor = "#A9F548FF",
			hideRecordIco = true,
			actor = 101512,
			actorName = "{playername}",
			say = "――そしたら、うちの城内で暴れてるやつらのことは？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 2,
			say = "むぅ……めんどいから……やっぱり指揮官のお城に行く",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			say = "でも、おねんねも……ちゃんとしないと……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 3,
			say = "じゃあ、城主指揮官……ウサウサに乗って？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			withoutActorName = true,
			hideRecordIco = true,
			actor = 101512,
			nameColor = "#A9F548FF",
			say = "ラフィーが両手を上げて伸びをすると、この世界観からだいぶ浮いている巨大なウサウサの背の上にこちらを乗せた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 3,
			say = "うん……これでいい……ウサウサ、しゅっぱーつ！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 4,
			say = "ラフィー、おねむ……城主指揮官、一緒にねんねして……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 5,
			say = "一緒にねんねして……悩み、全部忘れよう……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			withoutActorName = true,
			hideRecordIco = true,
			actor = 101512,
			nameColor = "#A9F548FF",
			say = "柔らかな呼吸が頬をかすめ、ほんのり甘い香りを運んでくる。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101512,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_520",
			spine = true,
			side = 2,
			expression = 5,
			say = "Zzzz……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			portrait = "zhihuiguan",
			nameColor = "#A9F548FF",
			hideRecordIco = true,
			actor = 101512,
			actorName = "{playername}",
			say = "Zzzz……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_520",
			spine = true,
			withoutActorName = true,
			hideRecordIco = true,
			actor = 101512,
			nameColor = "#A9F548FF",
			say = "雲の上にいるかのような柔らかさの中で、ラフィーと共に夢郷へと旅立っていった――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
