return {
	fadeOut = 1.5,
	mode = 2,
	id = "JIZHANRENZHEZHICHENG17",
	placeholder = {
		"playername"
	},
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_142",
			say = "島風に送らせた支援要請を受け、援助が届いた。",
			bgm = "sk-az-pv2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_142",
			say = "手際の良さで名を馳せる公家の令嬢ガスコーニュが自ら輸送隊を率いて、大量の物資を携え城に駆けつけてくれた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 999012,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			withoutActorName = true,
			hideRecordIco = true,
			say = "そして散歩と共に、物資リストの受け渡しをしたいと言われたが、どうも別の意図があるようにも思えた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "応急修理用の資材、生存に必要な補給品及び各種建築資材……以上、物資の入庫を確認",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "輸送ルートの最適化……完了。以降の供給効率の最大化を確保",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "主、物資の集計が完了した。ご確認を",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_142",
			portrait = "zhihuiguan",
			nameColor = "#A9F548FF",
			hideRecordIco = true,
			actor = 999012,
			actorName = "{playername}",
			say = "――予定では精鉄の矢じりは3000本だったはずだが、実際には7000本も入庫されてるのか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "余分は、ガスコーニュからの気持ち",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 999012,
			side = 2,
			bgName = "star_level_bg_142",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "追加の支援物資はまだ輸送中と認む",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "ガスコーニュがついているので、心配は無用",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			say = "そうして一通りチェックし終わると、城内の情勢は明らかに好転した。",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			say = "緊張がいくらか緩むと、庭の美しい景色も自然と目に映り込む。",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			say = "涼しい風が庭を吹き抜け、紅葉が空をゆるりと舞い、池へ落ちては幾重にも波紋を広げていった。",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "しかし、今回ガスコーニュが来た目的は、物資の引き渡しだけではない……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			say = "ガスコーニュは歩みを止め、心の奥まで見通すような穏やかな目で見つめてきた。",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "過去の情報に基づくと、主が今最も必要としているのは、より多くの物資と心の保障……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "……そして「応援」と「付き添い」と分析する",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "どうしてその結論に？",
					flag = 1
				},
				{
					content = "というと？",
					flag = 2
				}
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "主に関する情報は至るところに存在する",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "ガスコーニュはただ、主と「感情」と「欲求」に特別関心を持ってるだけ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "ガスコーニュはずっと主の動向を見守っていた",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "ご要望があれば、なんなりとお申し付けを。ガスコーニュがそれを実現する",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 999012,
			side = 2,
			bgName = "star_level_bg_142",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "物資というのは、視認可能かつ、容易に移転できる一種の媒介に過ぎない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 999012,
			side = 2,
			bgName = "star_level_bg_142",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "つまり消費され、更新されていくもの",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "しかし、その中に込められた「応えたい気持ち」――それだけは変わらない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "「応えたいもの」が具体的に何であるべきかについては、ガスコーニュも長い間思考を重ねてきた",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 999012,
			side = 2,
			bgName = "star_level_bg_142",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "最後に導き出したのは、簡単な結論だった",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "それは「付き添い」。しかも一時的ではなく、長く、持続的であるべき",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			say = "華美な修辞が一切なく、その素直さに応えずにはいられない言葉。",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_142",
			actor = 999012,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			say = "ガスコーニュはそっと自分のそばまで歩み寄ってきた。",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "ガスコーニュはどんな時でも主のそばに。必要とあらばどんな立ち位置でも",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "指示がなくともは主を守り、支え続ける。いつまでも変わらない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 999012,
			side = 2,
			bgName = "star_level_bg_142",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "主がどこへ向かうとしても、ガスコーニュに同行の許可を",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_142",
			actorName = "{playername}",
			actor = 999012,
			nameColor = "#A9F548FF",
			say = "――ありがとう。ガスコーニュの応援は、かけがえのない宝だ",
			hideRecordIco = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "主からのフィードバックを確認",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "ガスコーニュが感じているこの感情は……「喜び」……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "すごく特別な…感触……でも……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "……ガスコーニュ、コンディションの良好を認む",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "もし可能であれば、ガスコーニュはもっと知りたい",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_142",
			side = 2,
			dir = 1,
			actor = 999012,
			say = "引き続きガスコーニュに……主の「喜び」の入力を求む",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
