local var_0_0 = class("IslandShipAttrUpgradeCommand", pm.SimpleCommand)

function var_0_0.execute(arg_1_0, arg_1_1)
	local var_1_0 = arg_1_1:getBody()
	local var_1_1 = var_1_0.id
	local var_1_2 = getProxy(IslandProxy):GetIsland():GetCharacterAgency():GetShipById(var_1_1)

	if not var_1_2 then
		return
	end

	local var_1_3 = var_1_0.attrKy
	local var_1_4 = {}

	for iter_1_0, iter_1_1 in pairs(var_1_0.list or {}) do
		table.insert(var_1_4, {
			id = iter_1_0,
			num = iter_1_1
		})
	end

	pg.ConnectionMgr.GetInstance():Send(21605, {
		ship_id = var_1_1,
		type = var_1_3,
		item_list = var_1_4
	}, 21606, function(arg_2_0)
		if arg_2_0.result == 0 then
			local var_2_0 = Clone(var_1_2)
			local var_2_1 = getProxy(IslandProxy):GetIsland():GetInventoryAgency()
			local var_2_2 = 0

			for iter_2_0, iter_2_1 in pairs(var_1_0.list or {}) do
				local var_2_3 = IslandItem.New({
					id = iter_2_0
				})

				var_2_2 = var_2_2 + tonumber(var_2_3:GetUseArg()) * iter_2_1

				var_2_1:RemoveItem(iter_2_0, iter_2_1)
			end

			local var_2_4 = IslandShipAttr.GetAtrrName(var_1_3)

			var_1_2:AddExtraAttr(var_2_4, var_2_2)
			arg_1_0:sendNotification(GAME.ISLNAD_SHIP_ATTR_UPGRADE_DONE)
			IslandAchievementHelper.OnShipAttrUpgrade(var_2_0, var_1_2)
			pg.GameTrackerMgr.GetInstance():Record(GameTrackerBuilder.BuildIslandShipAttrUpgrade(var_2_0, var_1_2))
		else
			pg.TipsMgr.GetInstance():ShowTips(ERROR_MESSAGE[arg_2_0.result] .. arg_2_0.result)
		end
	end)
end

return var_0_0
