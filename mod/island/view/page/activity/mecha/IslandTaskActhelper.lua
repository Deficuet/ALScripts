...
function var_0_0.GetIslandTaskState(arg_1_0)
	local var_1_0 = getProxy(IslandProxy):GetIsland():GetTaskAgency()
	local var_1_1 = 0
	local var_1_2 = IslandTask.New({
		id = arg_1_0,
		process_list = {}
	}):GetTargetList()[1]

	assert(var_1_2, "target no exist")

	local var_1_3 = var_1_2:GetTargetNum()
	local var_1_4 = var_0_0.TASK_STATET_NORMAL
	local var_1_5 = var_1_0:GetTask(arg_1_0)

	if var_1_0:IsFinishTask(arg_1_0) then
		var_1_4 = var_0_0.TASK_STATET_RECIVED
		var_1_1 = var_1_3
	elseif var_1_5 and var_1_5:IsFinish() then
		var_1_4 = var_0_0.TASK_STATET_FINISH
		var_1_1 = var_1_3
	end

	if var_1_5 then
		var_1_1 = var_1_5:GetTargetList()[1]:GetProgress()
	end
...
function var_0_0.GetNDay(arg_2_0)
	...
	return math.min(#var_2_0, math.min(var_2_1, var_2_2))
end
...
function var_0_0.FirstEnter()
	...
	return PlayerPrefs.GetInt(var_0_1 .. var_5_0, 0) == 0
end

function var_0_0.SetNonFirstEnter(arg_6_0)
	local var_6_0 = getProxy(PlayerProxy):getRawData().id

	PlayerPrefs.SetInt(var_0_1 .. var_6_0, 1)
	PlayerPrefs.Save()

	local var_6_1 = getProxy(ActivityProxy):RawGetActivityById(arg_6_0)

	if var_6_1 then
		pg.m02:sendNotification(ActivityProxy.ACTIVITY_UPDATED, var_6_1)
	end
end
...
function var_0_0.ShouldTipIslandTask(arg_7_0)
	...
	local var_7_0 = var_0_0.GetNDay(arg_7_0)
	local var_7_1 = arg_7_0:getIslandConfig("config_data")[var_7_0] or {}

	return var_0_0.FirstEnter() or _.any(var_7_1, function(arg_8_0)
...
function var_0_0._TriggerTasks(arg_9_0)
	...
	local var_9_1 = _.flatten(var_9_0)
	local var_9_2 = {}
	local var_9_3 = getProxy(IslandProxy):GetIsland():GetTaskAgency()

	for iter_9_0, iter_9_1 in ipairs(var_9_1) do
		local var_9_4 = var_9_3:GetTask(iter_9_1)

		if not var_9_3:IsFinishTask(iter_9_1) and not var_9_4 then
...
function var_0_0.TriggerActTasks(arg_10_0)
	local var_10_0 = getProxy(ActivityProxy):getActivitiesByType(ActivityConst.ACTIVITY_TYPE_TASK_LIST)
