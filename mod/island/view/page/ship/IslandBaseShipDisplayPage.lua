local var_0_0 = class("IslandBaseShipDisplayPage", import("...base.IslandBasePage"))

function var_0_0.Ctor(arg_1_0, ...)
	var_0_0.super.Ctor(arg_1_0, ...)

	arg_1_0.displayUnit = IslandDisplayShipUnit.New()
	arg_1_0.gcCounter = 0
end

function var_0_0.Preload(arg_2_0, arg_2_1)
	arg_2_0:PrepareCharacterScene(arg_2_1)
end

function var_0_0.PrepareCharacterScene(arg_3_0, arg_3_1)
	arg_3_0.isLoadCharacterScene = true

	seriesAsync({
		function(arg_4_0)
			arg_3_0:CreateCharacterContainer()
			arg_3_0:CreateToolContainer()
			arg_3_0:LoadCharacterScene(arg_4_0)
		end,
		function(arg_5_0)
			arg_3_0:ModifyCameraMask()
			arg_3_0:ActivityCharacterCamera()
			arg_3_0:InitSceneTimeline()
			arg_5_0()
		end
	}, arg_3_1)
end

function var_0_0.CreateCharacterContainer(arg_6_0)
	arg_6_0.roleContainer = GameObject.New("roleContainer").transform

	pg.ViewUtils.SetLayer(arg_6_0.roleContainer, Layer.Character3D)
end

function var_0_0.LoadCharacterScene(arg_7_0, arg_7_1)
	local var_7_0 = "island/scenesres/scenes/character/map_shipmainui_scene"

	SceneOpMgr.Inst:LoadSceneAsyncWithProgress(var_7_0, "map_shipmainui", LoadSceneMode.Additive, function(arg_8_0)
		if arg_8_0 == 1 then
			arg_7_1()
		end
	end)
end

function var_0_0.ModifyCameraMask(arg_9_0)
	local var_9_0 = IslandCameraMgr.instance

	if IsNil(var_9_0) then
		var_9_0 = CheatTavernCameraMgr.instance
	end

	local var_9_1 = var_9_0._mainCamera

	arg_9_0.defaultCullingMask = var_9_1.cullingMask

	LuaHelper.SetCamCullingMask(var_9_1, "Character3D")
end

function var_0_0.ActivityCharacterCamera(arg_10_0)
	local var_10_0 = arg_10_0:GetActiveCamName()
	local var_10_1 = IslandCameraMgr.instance

	if IsNil(var_10_1) then
		var_10_1 = CheatTavernCameraMgr.instance
	end

	local var_10_2 = var_10_1:GetVirtualCamera(var_10_0)

	var_10_2.Follow = arg_10_0.roleContainer
	var_10_2.LookAt = arg_10_0.roleContainer

	var_10_1:ActiveVirtualCamera(var_10_0)
end

function var_0_0.InitSceneTimeline(arg_11_0)
	local var_11_0 = GameObject.Find("[sequence]")

	if var_11_0 then
		local var_11_1 = var_11_0:GetComponent(typeof(UnityEngine.Playables.PlayableDirector))

		TimelineSupport.DynamicBinding(var_11_1)
		var_11_1:Play()
	end
end

function var_0_0.ClearCharacterScene(arg_12_0, arg_12_1)
	if arg_12_0.isLoadCharacterScene then
		arg_12_0:UnLoadCharacterScene(arg_12_1)
		arg_12_0:ClearCharacterContainer()
		arg_12_0:ClearToolContainer()
		arg_12_0:ResetCameraMask()
		arg_12_0:ActivityPlayerCamera()
		arg_12_0:emitCore(ISLAND_EVT.REFRESH_WEATHER_SYSTEM)
	end

	arg_12_0.isLoadCharacterScene = false
end

function var_0_0.OnHome(arg_13_0)
	arg_13_0:ClearCharacterScene(function()
		arg_13_0:emit(BaseUI.ON_HOME)
	end)
end

function var_0_0.LoadCharacter(arg_15_0, arg_15_1, arg_15_2)
	arg_15_0:UnloadCharacter(arg_15_0.loadData)

	local var_15_0 = {
		isCommander = arg_15_2,
		modelData = arg_15_1
	}

	arg_15_0.loadData = var_15_0

	local function var_15_1(arg_16_0, arg_16_1)
		if var_15_0.modelData.model ~= arg_15_0.loadData.modelData.model then
			arg_15_0:UnloadCharacter(var_15_0)

			return
		end

		arg_15_0.role = arg_16_0

		pg.ViewUtils.SetLayer(arg_15_0.role.transform, Layer.Character3D)
		setParent(arg_15_0.role, arg_15_0.roleContainer)

		arg_15_0.role.transform.eulerAngles = Vector3(0, 180, 0)

		local var_16_0 = 0
		local var_16_1 = arg_15_0._tf.rect.width / arg_15_0._tf.rect.height

		if var_16_1 < 1.7777777777777777 then
			var_16_0 = 0.5 * (1.7777777777777777 - var_16_1) / 0.4444444444444444
		end

		arg_15_0.role.transform.localPosition = Vector3(var_16_0, 0, 0)

		local var_16_2 = arg_15_0:GetSmoothRotateObject()
		local var_16_3 = GetOrAddComponent(var_16_2, typeof(SmoothRotateObject))

		var_16_3:SetUp(arg_15_0.role.transform)

		var_16_3.rotationSpeed = pg.island_set.character_detail_camera_speed.key_value_int

		arg_15_0.displayUnit:OnAttach(arg_16_0, arg_15_0.toolContainer)

		if arg_16_1 and arg_16_1 ~= "" then
			local var_16_4 = GetOrAddComponent(arg_15_0.role.transform:GetChild(0), typeof(Animator))

			for iter_16_0 = 1, var_16_4.layerCount do
				var_16_4:CrossFadeInFixedTime(arg_16_1, 0, iter_16_0 - 1)
			end
		end

		arg_15_0:OnCharLoaded(var_15_0.modelData)
	end

	arg_15_0:_LoadModel(var_15_0, var_15_1)
end

function var_0_0._LoadModel(arg_17_0, arg_17_1, arg_17_2)
	pg.UIMgr.GetInstance():LoadingOn()

	local var_17_0 = arg_17_1.modelData

	if arg_17_1.isCommander then
		arg_17_0:GetPoolMgr():GetCommanderModel(var_17_0, function(arg_18_0)
			arg_17_2(arg_18_0, var_17_0.personal_ani)
			pg.UIMgr.GetInstance():LoadingOff()
		end)
	else
		arg_17_0:GetPoolMgr():GetCharacter(var_17_0.model, var_17_0.animator, function(arg_19_0)
			arg_17_2(arg_19_0, var_17_0.personal_ani)
			pg.UIMgr.GetInstance():LoadingOff()
		end)
	end
end

function var_0_0.UnloadCharacter(arg_20_0, arg_20_1)
	if not arg_20_1 then
		return
	end

	local var_20_0 = arg_20_1.modelData
	local var_20_1 = arg_20_1.isCommander
	local var_20_2 = arg_20_0:GetSmoothRotateObject():GetComponent(typeof(SmoothRotateObject))

	if var_20_2 then
		Object.Destroy(var_20_2)

		local var_20_3
	end

	if arg_20_0.role then
		arg_20_0.displayUnit:OnDetach()
		pg.ViewUtils.SetLayer(arg_20_0.role.transform, Layer.Default)

		if arg_20_0.isCommander then
			arg_20_0:GetPoolMgr():ReturnCommanderModel(arg_20_0.role)
		else
			arg_20_0:GetPoolMgr():ReturnCharacter(var_20_0.model, var_20_0.animator, arg_20_0.role)
		end

		arg_20_0.role = nil
	end

	if arg_20_0.gcCounter >= 3 then
		arg_20_0.gcCounter = 0

		IslandHelper.RunGC(true)
	else
		arg_20_0.gcCounter = arg_20_0.gcCounter + 1
	end
end

function var_0_0.ClearCharacterContainer(arg_21_0)
	arg_21_0:UnloadCharacter(arg_21_0.loadData)

	arg_21_0.loadData = nil

	if not IsNil(arg_21_0.roleContainer) then
		Object.Destroy(arg_21_0.roleContainer.gameObject)

		arg_21_0.roleContainer = nil
	end
end

function var_0_0.UnLoadCharacterScene(arg_22_0, arg_22_1)
	local var_22_0 = "island/scenesres/scenes/character/map_shipmainui_scene"

	SceneOpMgr.Inst:UnloadSceneAsync(var_22_0, "map_shipmainui", function()
		if arg_22_1 then
			arg_22_1()
		end
	end)
end

function var_0_0.ResetCameraMask(arg_24_0)
	local var_24_0 = IslandCameraMgr.instance

	if IsNil(var_24_0) then
		var_24_0 = CheatTavernCameraMgr.instance
	end

	if arg_24_0.defaultCullingMask and var_24_0 then
		local var_24_1 = var_24_0._mainCamera

		LuaHelper.ResetCamCullingMask(var_24_1, arg_24_0.defaultCullingMask)
	end
end

function var_0_0.ActivityPlayerCamera(arg_25_0)
	if not IslandCameraMgr.instance then
		return
	end

	IslandCameraMgr.instance:ActiveVirtualCamera(IslandConst.FOLLOW_CAMERA_NAME)
end

function var_0_0.CreateToolContainer(arg_26_0)
	arg_26_0.toolContainer = GameObject.New("toolContainer").transform

	pg.ViewUtils.SetLayer(arg_26_0.roleContainer, Layer.Default)
end

function var_0_0.ClearToolContainer(arg_27_0)
	if not IsNil(arg_27_0.toolContainer) then
		Object.Destroy(arg_27_0.toolContainer.gameObject)

		arg_27_0.toolContainer = nil
	end
end

function var_0_0.OnClearItemAnimator(arg_28_0)
	arg_28_0.displayUnit:OnClearItemAnimator()
end

function var_0_0.Hide(arg_29_0)
	var_0_0.super.Hide(arg_29_0)
	arg_29_0:ClearCharacterScene()
end

function var_0_0.OnDisable(arg_30_0)
	arg_30_0:ClearCharacterScene()
end

function var_0_0.OnDestroy(arg_31_0)
	arg_31_0:ClearCharacterScene()

	for iter_31_0, iter_31_1 in pairs(arg_31_0.cards or {}) do
		iter_31_1:Dispose()
	end

	arg_31_0.cards = nil
end

function var_0_0.GetActiveCamName(arg_32_0)
	return IslandConst.CHARA_CAMERA_NAME
end

function var_0_0.GetSmoothRotateObject(arg_33_0)
	assert(false, "Write me")
end

function var_0_0.OnCharLoaded(arg_34_0)
	return
end

function var_0_0.Hide(arg_35_0, arg_35_1, arg_35_2)
	var_0_0.super.Hide(arg_35_0, arg_35_1, arg_35_2)
	IslandHelper.RunGC(true)
end

return var_0_0
