local var_0_0 = class("NewSkinAtlasLayer", import("...base.BaseUI"))
local var_0_1 = -1
local var_0_2 = 9999

function var_0_0.getUIName(arg_1_0)
	return "NewSkinAtlasUI"
end

function var_0_0.init(arg_2_0)
	arg_2_0.bg = arg_2_0._tf:Find("bg")
	arg_2_0.empty = arg_2_0._tf:Find("empty")
	arg_2_0.backBtn = arg_2_0._tf:Find("adapt/top/closeBtn")
	arg_2_0.homeBtn = arg_2_0._tf:Find("adapt/top/homeBtn")
	arg_2_0.resources = arg_2_0._tf:Find("adapt/top/resources")
	arg_2_0.filterBtn = arg_2_0._tf:Find("adapt/top/filterBtn")
	arg_2_0.search = arg_2_0._tf:Find("adapt/top/search")
	arg_2_0.scrollrect = arg_2_0._tf:Find("scroll"):GetComponent("LScrollRect")
	arg_2_0.filterUI = arg_2_0._tf:Find("subPage/filterUI")
	arg_2_0.filterContent = arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content")

	setActive(arg_2_0.filterUI, false)
	setText(arg_2_0.empty:Find("Text"), i18n("shop_new_unfound"))
	setText(arg_2_0._tf:Find("adapt/top/title/Text"), i18n("shop_new_shop"))
	setText(arg_2_0._tf:Find("adapt/top/have/Text"), i18n("shop_new_owned_skin"))
	setText(arg_2_0.filterBtn:Find("Text"), i18n("shop_new_sort"))
	setText(arg_2_0.search:Find("holder"), i18n("shop_new_search"))
	setText(arg_2_0.filterUI:Find("panel/title"), i18n("shop_new_sort"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/own/subTitleFrame/subTitle"), i18n("shop_new_review"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/own/options/0/Text"), i18n("shop_new_all"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/own/options/1/Text"), i18n("shop_new_unused"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/subTitleFrame/subTitle"), i18n("shop_new_type"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/options/0/Text"), i18n("shop_new_all"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/options/2/Text"), i18n("shop_new_static"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/options/3/Text"), i18n("shop_new_dynamic"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/options/4/Text"), i18n("shop_new_static_bg"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/options/5/Text"), i18n("shop_new_dynamic_bg"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/type/options/6/Text"), i18n("shop_new_bgm"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/shipHave/subTitleFrame/subTitle"), i18n("shop_new_index"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/shipHave/options/0/Text"), i18n("shop_new_all"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/shipHave/options/1/Text"), i18n("shop_new_ship_owned"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/shipHave/options/2/Text"), i18n("shop_new_ship_havent_owned"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/camp/subTitleFrame/subTitle"), i18n("shop_new_nation"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/rarity/subTitleFrame/subTitle"), i18n("shop_new_rarity"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/shipType/subTitleFrame/subTitle"), i18n("shop_new_category"))
	setText(arg_2_0.filterUI:Find("panel/filterScroll/Viewport/Content/themeType/subTitleFrame/subTitle"), i18n("shop_new_skin_theme"))
	setText(arg_2_0.filterUI:Find("panel/bottom/ok/Text"), i18n("shop_new_confirm"))
	pg.UIMgr.GetInstance():OverlayPanel(arg_2_0._tf, {
		pbList = {
			arg_2_0.bg,
			arg_2_0.filterUI:Find("panel")
		}
	})
end

function var_0_0.didEnter(arg_3_0)
	arg_3_0:InitData()
	arg_3_0:SetFilterPanel()
	arg_3_0:SetResource()
	arg_3_0:SetSkinScroll()
	arg_3_0:Refresh()
	onButton(arg_3_0, arg_3_0.backBtn, function()
		arg_3_0:closeView()
	end, SFX_CANCEL)
	onButton(arg_3_0, arg_3_0.homeBtn, function()
		arg_3_0:emit(var_0_0.ON_HOME)
	end, SFX_CANCEL)
	onButton(arg_3_0, arg_3_0.filterBtn, function()
		arg_3_0:OpenFilterPanel()
	end, SFX_PANEL)
	onInputChanged(arg_3_0, arg_3_0.search, function()
		arg_3_0:Refresh()

		local var_7_0 = getInputText(arg_3_0.search)

		setActive(arg_3_0.search:Find("holder"), var_7_0 == "")
	end)
end

function var_0_0.InitData(arg_8_0)
	arg_8_0.skins = getProxy(ShipSkinProxy):GetOwnSkins()

	arg_8_0:GetSkinClassify()

	arg_8_0.filterValues = {
		ownType = 0,
		shipHaveType = 0,
		typeType = {
			0
		},
		campType = {
			0
		},
		rarityType = {
			0
		},
		shipType = {
			0
		},
		themeType = {
			0
		}
	}
	arg_8_0.filterValuesTemp = Clone(arg_8_0.filterValues)
end

function var_0_0.SetResource(arg_9_0)
	local var_9_0 = getProxy(PlayerProxy):getRawData()

	setText(arg_9_0.resources:Find("gem/Text"), var_9_0:getTotalGem())
	onButton(arg_9_0, arg_9_0.resources:Find("gem"), function()
		pg.playerResUI:ClickGem()
	end, SFX_PANEL)
end

function var_0_0.SetSkinScroll(arg_11_0)
	arg_11_0.scrollrect.isNewLoadingMethod = true

	function arg_11_0.scrollrect.onInitItem(arg_12_0)
		arg_11_0:OnInitItem(arg_12_0)
	end

	function arg_11_0.scrollrect.onUpdateItem(arg_13_0, arg_13_1)
		arg_11_0:OnUpdateItem(arg_13_0, arg_13_1)
	end

	function arg_11_0.scrollrect.onReturnItem(arg_14_0, arg_14_1)
		arg_11_0:OnReturnItem(arg_14_0, arg_14_1)
	end
end

function var_0_0.OnInitItem(arg_15_0, arg_15_1)
	ClearTweenItemAlphaAndWhite(arg_15_1)
end

function var_0_0.ReturnIndex(arg_16_0, arg_16_1)
	local var_16_0 = arg_16_0.scrollShowClassifyIds[arg_16_1]

	if arg_16_0.indexDic[arg_16_1] then
		for iter_16_0 = #arg_16_0.groupDic[var_16_0], 1, -1 do
			if arg_16_0.groupDic[var_16_0][iter_16_0] == arg_16_1 then
				table.remove(arg_16_0.groupDic[var_16_0], iter_16_0)
			end
		end
	end

	arg_16_0.indexDic[arg_16_1] = false
end

function var_0_0.RegisterIndex(arg_17_0, arg_17_1)
	arg_17_0.indexDic[arg_17_1] = true

	local var_17_0 = arg_17_0.scrollShowClassifyIds[arg_17_1]

	arg_17_0.groupDic[var_17_0] = arg_17_0.groupDic[var_17_0] or {}

	table.insert(arg_17_0.groupDic[var_17_0], arg_17_1)
end

function var_0_0.ChangeClassifyName(arg_18_0, arg_18_1)
	for iter_18_0, iter_18_1 in ipairs(arg_18_1) do
		local var_18_0

		for iter_18_2, iter_18_3 in pairs(arg_18_0.goDic) do
			if iter_18_1 == iter_18_3 then
				var_18_0 = iter_18_2

				break
			end
		end

		if var_18_0 then
			local var_18_1 = arg_18_0.scrollShowClassifyIds[iter_18_1]
			local var_18_2 = arg_18_0.classifyNames[table.indexof(arg_18_0.classifyIds, var_18_1)]
			local var_18_3 = (arg_18_0.groupDic[var_18_1] or {})[1] == iter_18_1

			setActive(tf(var_18_0):Find("titleBar"), var_18_3)

			if var_18_3 then
				setText(tf(var_18_0):Find("titleBar/title"), var_18_2)

				local var_18_4 = arg_18_0._tf:Find("scroll/content"):GetComponent(typeof(VerticalLayoutGroup)).spacing
				local var_18_5 = arg_18_0.scrollClassifyNum[var_18_1]
				local var_18_6 = tf(var_18_0):GetComponent(typeof(LayoutElement)).preferredHeight
				local var_18_7 = var_18_6 * var_18_5 + var_18_4 * (var_18_5 - 1)
				local var_18_8 = (arg_18_0.scrollShouldShowName[iter_18_1] - 1) * (var_18_6 + var_18_4)

				setSizeDelta(tf(var_18_0):Find("titleBar"), {
					x = tf(var_18_0):Find("titleBar").rect.width,
					y = var_18_7
				})
				setAnchoredPosition(tf(var_18_0):Find("titleBar"), {
					y = var_18_8
				})
			end
		end
	end
end

function var_0_0.GetDisplayIndex(arg_19_0, arg_19_1)
	return (arg_19_0.groupDic[arg_19_1] or {})[1]
end

function var_0_0.ClickTrigger(arg_20_0, arg_20_1, arg_20_2)
	arg_20_0:emit(NewSkinAtlasMediator.OPEN_SHOW_LAYER, arg_20_2)
end

function var_0_0.OnUpdateItem(arg_21_0, arg_21_1, arg_21_2)
	TweenItemAlphaAndWhite(arg_21_2)

	arg_21_1 = arg_21_1 + 1

	local var_21_0 = arg_21_0.scrollDisplays[arg_21_1]

	if arg_21_0.goDic[arg_21_2] and arg_21_0.goDic[arg_21_2] ~= arg_21_1 then
		local var_21_1 = arg_21_0.scrollShowClassifyIds[arg_21_0.goDic[arg_21_2]]
		local var_21_2 = arg_21_0:GetDisplayIndex(var_21_1)

		arg_21_0:ReturnIndex(arg_21_0.goDic[arg_21_2])

		if var_21_2 ~= arg_21_0:GetDisplayIndex(var_21_1) then
			local var_21_3 = {}

			table.insert(var_21_3, var_21_2)
			table.insert(var_21_3, arg_21_0:GetDisplayIndex(var_21_1))
			arg_21_0:ChangeClassifyName(var_21_3)
		end
	end

	arg_21_0.goDic[arg_21_2] = arg_21_1

	local var_21_4 = arg_21_0.scrollShowClassifyIds[arg_21_1]
	local var_21_5 = arg_21_0:GetDisplayIndex(var_21_4)

	arg_21_0:RegisterIndex(arg_21_1)

	local var_21_6 = {}

	if var_21_5 ~= arg_21_0:GetDisplayIndex(var_21_4) then
		table.insert(var_21_6, var_21_5)
	end

	table.insert(var_21_6, arg_21_1)
	arg_21_0:ChangeClassifyName(var_21_6)

	local var_21_7 = UIItemList.New(tf(arg_21_2):Find("skins"), tf(arg_21_2):Find("skins/SkinAtlasCard"))

	var_21_7:make(function(arg_22_0, arg_22_1, arg_22_2)
		if arg_22_0 == UIItemList.EventUpdate then
			local var_22_0 = var_21_0[arg_22_1 + 1]
			local var_22_1 = SkinAtlasCard.New(arg_22_2)

			table.insert(arg_21_0.cards, var_22_1)
			var_22_1:Update(var_22_0, arg_22_1 + 1)
			onButton(arg_21_0, arg_22_2, function()
				arg_21_0:ClickTrigger(var_22_1, var_22_0)
			end, SFX_PANEL)
			onButton(arg_21_0, var_22_1.changeSkinUI, function()
				var_22_1:changeSkinNext()
			end, SFX_PANEL)
		end
	end)
	var_21_7:align(#var_21_0)
end

function var_0_0.OnReturnItem(arg_25_0, arg_25_1, arg_25_2)
	ClearTweenItemAlphaAndWhite(arg_25_2)

	if arg_25_0.exited then
		return
	end

	if arg_25_0.goDic[arg_25_2] then
		local var_25_0 = arg_25_0.scrollShowClassifyIds[arg_25_0.goDic[arg_25_2]]
		local var_25_1 = arg_25_0:GetDisplayIndex(var_25_0)

		arg_25_0:ReturnIndex(arg_25_0.goDic[arg_25_2])

		if var_25_1 ~= arg_25_0:GetDisplayIndex(var_25_0) then
			local var_25_2 = {}

			table.insert(var_25_2, var_25_1)
			table.insert(var_25_2, arg_25_0:GetDisplayIndex(var_25_0))
			arg_25_0:ChangeClassifyName(var_25_2)
		end
	end
end

function var_0_0.Refresh(arg_26_0)
	arg_26_0.showClassifyIds = {}
	arg_26_0.displays = {}

	local var_26_0 = getInputText(arg_26_0.search)
	local var_26_1 = Clone(arg_26_0.classifyIds)
	local var_26_2 = Clone(arg_26_0.classifyNames)

	table.remove(var_26_1, 1)
	table.remove(var_26_2, 1)

	for iter_26_0, iter_26_1 in pairs(arg_26_0.skins) do
		if arg_26_0:filterOk(iter_26_1) and arg_26_0:IsSearchType(var_26_0, iter_26_1) then
			local var_26_3 = arg_26_0:GetShopTypeIdBySkinId(iter_26_1.id)
			local var_26_4 = var_26_3 == 0 and var_0_2 or var_26_3

			if not arg_26_0.displays[var_26_4] then
				arg_26_0.displays[var_26_4] = {}
			end

			table.insert(arg_26_0.displays[var_26_4], iter_26_1)
		end
	end

	for iter_26_2, iter_26_3 in ipairs(var_26_1) do
		if arg_26_0.displays[iter_26_3] then
			table.insert(arg_26_0.showClassifyIds, iter_26_3)
		end
	end

	setActive(arg_26_0.empty, #arg_26_0.showClassifyIds == 0)

	arg_26_0.scrollShowClassifyIds = {}
	arg_26_0.scrollDisplays = {}
	arg_26_0.scrollShouldShowName = {}
	arg_26_0.scrollClassifyNum = {}

	for iter_26_4, iter_26_5 in ipairs(arg_26_0.showClassifyIds) do
		local var_26_5 = Clone(arg_26_0.displays[iter_26_5])
		local var_26_6 = false

		arg_26_0.scrollClassifyNum[iter_26_5] = math.ceil(#var_26_5 / 8)

		local var_26_7 = 1

		while #var_26_5 > 8 do
			table.insert(arg_26_0.scrollShowClassifyIds, iter_26_5)
			table.insert(arg_26_0.scrollShouldShowName, var_26_7)

			var_26_7 = var_26_7 + 1
			var_26_6 = var_26_6 or true

			local var_26_8 = {}

			for iter_26_6 = 1, 8 do
				table.insert(var_26_8, table.remove(var_26_5, 1))
			end

			table.insert(arg_26_0.scrollDisplays, var_26_8)
		end

		if #var_26_5 > 0 then
			table.insert(arg_26_0.scrollShowClassifyIds, iter_26_5)
			table.insert(arg_26_0.scrollShouldShowName, var_26_7)

			local var_26_9

			var_26_9 = var_26_6 or true

			table.insert(arg_26_0.scrollDisplays, var_26_5)
		end
	end

	arg_26_0.indexDic = {}
	arg_26_0.groupDic = {}
	arg_26_0.goDic = {}

	if arg_26_0.cards then
		for iter_26_7, iter_26_8 in ipairs(arg_26_0.cards) do
			iter_26_8:Dispose()
		end
	end

	arg_26_0.cards = {}

	arg_26_0.scrollrect:SetTotalCount(#arg_26_0.scrollShowClassifyIds, 0)
end

function var_0_0.IsSearchType(arg_27_0, arg_27_1, arg_27_2)
	if not arg_27_1 or arg_27_1 == "" then
		return true
	end

	local var_27_0 = arg_27_2.id

	return ShipSkin.New({
		id = var_27_0
	}):IsMatchKey(arg_27_1)
end

function var_0_0.SetFilterPanel(arg_28_0)
	local var_28_0 = arg_28_0.filterContent:Find("own/options")
	local var_28_1 = arg_28_0.filterContent:Find("type/options")
	local var_28_2 = arg_28_0.filterContent:Find("shipHave/options")
	local var_28_3 = arg_28_0.filterContent:Find("camp/options")
	local var_28_4 = arg_28_0.filterContent:Find("rarity/options")
	local var_28_5 = arg_28_0.filterContent:Find("shipType/options")
	local var_28_6 = arg_28_0.filterContent:Find("themeType/options")

	arg_28_0:SetOptionList(var_28_3, ShipIndexConst.CampNames, true)
	arg_28_0:SetOptionList(var_28_4, ShipIndexConst.RarityNames, true)
	arg_28_0:SetOptionList(var_28_5, ShipIndexConst.TypeNames, true)
	arg_28_0:SetOptionList(var_28_6, arg_28_0.classifyNames)
	arg_28_0:SetSingleOptions(var_28_0, "ownType")
	arg_28_0:SetMultiOptions(var_28_1, "typeType")
	arg_28_0:SetSingleOptions(var_28_2, "shipHaveType")
	arg_28_0:SetMultiOptions(var_28_3, "campType")
	arg_28_0:SetMultiOptions(var_28_4, "rarityType")
	arg_28_0:SetMultiOptions(var_28_5, "shipType")
	arg_28_0:SetMultiOptions(var_28_6, "themeType")
	onButton(arg_28_0, arg_28_0.filterUI:Find("bg"), function()
		for iter_29_0, iter_29_1 in pairs(arg_28_0.filterValues) do
			arg_28_0.filterValuesTemp[iter_29_0] = Clone(arg_28_0.filterValues[iter_29_0])
		end

		setActive(arg_28_0.filterUI, false)
	end, SFX_PANEL)
	onButton(arg_28_0, arg_28_0.filterUI:Find("panel/closeBtn"), function()
		for iter_30_0, iter_30_1 in pairs(arg_28_0.filterValues) do
			arg_28_0.filterValuesTemp[iter_30_0] = Clone(arg_28_0.filterValues[iter_30_0])
		end

		setActive(arg_28_0.filterUI, false)
	end, SFX_PANEL)
	onButton(arg_28_0, arg_28_0.filterUI:Find("panel/bottom/ok"), function()
		for iter_31_0, iter_31_1 in pairs(arg_28_0.filterValues) do
			arg_28_0.filterValues[iter_31_0] = Clone(arg_28_0.filterValuesTemp[iter_31_0])
		end

		setActive(arg_28_0.filterUI, false)
		arg_28_0:Refresh()
	end, SFX_PANEL)
end

function var_0_0.OpenFilterPanel(arg_32_0)
	setActive(arg_32_0.filterUI, true)

	local var_32_0 = arg_32_0.filterContent:Find("own/options")
	local var_32_1 = arg_32_0.filterContent:Find("type/options")
	local var_32_2 = arg_32_0.filterContent:Find("shipHave/options")
	local var_32_3 = arg_32_0.filterContent:Find("camp/options")
	local var_32_4 = arg_32_0.filterContent:Find("rarity/options")
	local var_32_5 = arg_32_0.filterContent:Find("shipType/options")
	local var_32_6 = arg_32_0.filterContent:Find("themeType/options")

	arg_32_0:SetSingleOptions(var_32_0, "ownType", true)
	arg_32_0:SetMultiOptions(var_32_1, "typeType", true)
	arg_32_0:SetSingleOptions(var_32_2, "shipHaveType", true)
	arg_32_0:SetMultiOptions(var_32_3, "campType", true)
	arg_32_0:SetMultiOptions(var_32_4, "rarityType", true)
	arg_32_0:SetMultiOptions(var_32_5, "shipType", true)
	arg_32_0:SetMultiOptions(var_32_6, "themeType", true)
end

function var_0_0.SetOptionList(arg_33_0, arg_33_1, arg_33_2, arg_33_3)
	local var_33_0 = UIItemList.New(arg_33_1, arg_33_1:GetChild(0))

	var_33_0:make(function(arg_34_0, arg_34_1, arg_34_2)
		if arg_34_0 == UIItemList.EventUpdate then
			local var_34_0 = arg_33_2[arg_34_1 + 1]

			if arg_33_3 then
				var_34_0 = i18n(var_34_0)
			end

			arg_34_2.name = arg_34_1

			setText(arg_34_2:Find("Text"), var_34_0)
		end
	end)
	var_33_0:align(#arg_33_2)
end

function var_0_0.SetSingleOptions(arg_35_0, arg_35_1, arg_35_2, arg_35_3)
	for iter_35_0 = 0, arg_35_1.childCount - 1 do
		local var_35_0 = arg_35_1:GetChild(iter_35_0)

		arg_35_0:SetOptionSelect(arg_35_1:GetChild(iter_35_0), iter_35_0 == arg_35_0.filterValuesTemp[arg_35_2])

		if not arg_35_3 then
			onButton(arg_35_0, var_35_0, function()
				arg_35_0.filterValuesTemp[arg_35_2] = iter_35_0

				for iter_36_0 = 0, arg_35_1.childCount - 1 do
					arg_35_0:SetOptionSelect(arg_35_1:GetChild(iter_36_0), iter_36_0 == iter_35_0)
				end
			end, SFX_PANEL)
		end
	end
end

function var_0_0.SetMultiOptions(arg_37_0, arg_37_1, arg_37_2, arg_37_3)
	for iter_37_0 = 0, arg_37_1.childCount - 1 do
		local var_37_0 = arg_37_1:GetChild(iter_37_0)

		arg_37_0:SetOptionSelect(arg_37_1:GetChild(iter_37_0), table.contains(arg_37_0.filterValuesTemp[arg_37_2], iter_37_0))

		if not arg_37_3 then
			onButton(arg_37_0, var_37_0, function()
				if iter_37_0 == 0 then
					arg_37_0.filterValuesTemp[arg_37_2] = {
						0
					}

					for iter_38_0 = 0, arg_37_1.childCount - 1 do
						arg_37_0:SetOptionSelect(arg_37_1:GetChild(iter_38_0), iter_38_0 == 0)
					end
				else
					table.removebyvalue(arg_37_0.filterValuesTemp[arg_37_2], 0)

					if table.contains(arg_37_0.filterValuesTemp[arg_37_2], iter_37_0) then
						table.removebyvalue(arg_37_0.filterValuesTemp[arg_37_2], iter_37_0)
					else
						table.insert(arg_37_0.filterValuesTemp[arg_37_2], iter_37_0)
					end

					local var_38_0 = true

					for iter_38_1 = 1, arg_37_1.childCount - 1 do
						if not table.contains(arg_37_0.filterValuesTemp[arg_37_2], iter_38_1) then
							var_38_0 = false

							break
						end
					end

					if #arg_37_0.filterValuesTemp[arg_37_2] == 0 then
						var_38_0 = true
					end

					if var_38_0 then
						arg_37_0.filterValuesTemp[arg_37_2] = {
							0
						}
					end

					for iter_38_2 = 0, arg_37_1.childCount - 1 do
						arg_37_0:SetOptionSelect(arg_37_1:GetChild(iter_38_2), table.contains(arg_37_0.filterValuesTemp[arg_37_2], iter_38_2))
					end
				end
			end, SFX_PANEL)
		end
	end
end

function var_0_0.SetOptionSelect(arg_39_0, arg_39_1, arg_39_2)
	setActive(arg_39_1:Find("selectedFrame"), arg_39_2)

	local var_39_0 = arg_39_1:Find("Text"):GetComponent(typeof(Text))

	if arg_39_2 then
		var_39_0.color = Color.New(1, 1, 1, 1)
	else
		var_39_0.color = Color.New(0, 0, 0, 0.5)
	end
end

function var_0_0.GetSkinClassify(arg_40_0)
	arg_40_0.classifyIds = Clone(pg.skin_page_template.all)

	table.insert(arg_40_0.classifyIds, 1, var_0_1)

	arg_40_0.classifyNames = {}

	for iter_40_0, iter_40_1 in ipairs(arg_40_0.classifyIds) do
		if iter_40_1 == var_0_1 then
			table.insert(arg_40_0.classifyNames, "全部")
		else
			table.insert(arg_40_0.classifyNames, pg.skin_page_template[iter_40_1].name)
		end
	end
end

function var_0_0.filterOk(arg_41_0, arg_41_1)
	local var_41_0 = arg_41_0.filterValues.ownType
	local var_41_1 = arg_41_0.filterValues.typeType
	local var_41_2 = arg_41_0.filterValues.shipHaveType
	local var_41_3 = arg_41_0.filterValues.campType
	local var_41_4 = arg_41_0.filterValues.rarityType
	local var_41_5 = arg_41_0.filterValues.shipType
	local var_41_6 = arg_41_0.filterValues.themeType
	local var_41_7 = arg_41_1.id
	local var_41_8 = arg_41_1:GetDefaultShipConfig()
	local var_41_9 = arg_41_0:ToVShip(var_41_8)

	if var_41_0 ~= 0 then
		local var_41_10 = false
		local var_41_11 = getProxy(ShipSkinProxy):hasSkin(var_41_7)
		local var_41_12 = arg_41_1:NoUse()

		if var_41_0 == 1 and var_41_11 and var_41_12 then
			var_41_10 = true
		end

		if not var_41_10 then
			return false
		end
	end

	if var_41_1[1] ~= 0 then
		local var_41_13 = false

		for iter_41_0, iter_41_1 in ipairs(var_41_1) do
			if iter_41_1 == 1 and (arg_41_1:IsLive2d() or arg_41_1:IsLive2dPlus()) then
				var_41_13 = true
			end

			if iter_41_1 == 2 and not arg_41_1:IsLive2d() and not arg_41_1:IsLive2dPlus() and not arg_41_1:IsSpine() and not arg_41_1:IsSpinePlus() then
				var_41_13 = true
			end

			if iter_41_1 == 3 and (arg_41_1:IsSpine() or arg_41_1:IsSpinePlus()) then
				var_41_13 = true
			end

			if iter_41_1 == 4 and arg_41_1:IsBG() then
				var_41_13 = true
			end

			if iter_41_1 == 5 and arg_41_1:IsDbg() then
				var_41_13 = true
			end

			if iter_41_1 == 6 and arg_41_1:isBgm() then
				var_41_13 = true
			end

			if var_41_13 then
				break
			end
		end

		if not var_41_13 then
			return false
		end
	end

	if var_41_2 ~= 0 then
		local var_41_14 = false
		local var_41_15 = arg_41_1:CantUse()

		if var_41_2 == 1 and not var_41_15 then
			var_41_14 = true
		end

		if var_41_2 == 2 and var_41_15 then
			var_41_14 = true
		end

		if not var_41_14 then
			return false
		end
	end

	if var_41_3[1] ~= 0 then
		local var_41_16 = false

		for iter_41_2, iter_41_3 in ipairs(var_41_3) do
			local var_41_17 = ShipIndexCfg.camp

			for iter_41_4, iter_41_5 in ipairs(var_41_17[iter_41_3 + 1].types) do
				if iter_41_5 == Nation.LINK then
					if var_41_9:getNation() >= Nation.LINK then
						var_41_16 = true
					end
				elseif iter_41_5 == var_41_9:getNation() then
					var_41_16 = true
				end
			end

			if var_41_16 then
				break
			end
		end

		if not var_41_16 then
			return false
		end
	end

	if var_41_4[1] ~= 0 then
		local var_41_18 = false

		for iter_41_6, iter_41_7 in ipairs(var_41_4) do
			local var_41_19 = ShipIndexCfg.rarity

			if table.contains(var_41_19[iter_41_7 + 1].types, var_41_9:getRarity()) then
				var_41_18 = true
			end

			if var_41_18 then
				break
			end
		end

		if not var_41_18 then
			return false
		end
	end

	if var_41_5[1] ~= 0 then
		local var_41_20 = false

		for iter_41_8, iter_41_9 in ipairs(var_41_5) do
			local var_41_21 = ShipIndexCfg.type
			local var_41_22 = var_41_21[iter_41_9 + 1].types

			if iter_41_9 + 1 < 4 then
				local var_41_23 = var_41_21[iter_41_9].shipTypes

				if table.contains(var_41_22, var_41_9:getShipType()) then
					var_41_20 = true
				end

				if table.contains(var_41_22, var_41_9:getTeamType()) then
					var_41_20 = true
				end
			elseif table.contains(var_41_22, var_41_9:getShipType()) then
				var_41_20 = true
			end

			if var_41_20 then
				break
			end
		end

		if not var_41_20 then
			return false
		end
	end

	if var_41_6[1] ~= 0 then
		local var_41_24 = false

		for iter_41_10, iter_41_11 in ipairs(var_41_6) do
			local var_41_25 = arg_41_0.classifyIds[iter_41_11 + 1]

			if var_41_25 == var_0_1 then
				var_41_24 = true
			else
				local var_41_26 = arg_41_0:GetShopTypeIdBySkinId(var_41_7)

				var_41_24 = (var_41_26 == 0 and var_0_2 or var_41_26) == var_41_25
			end

			if var_41_24 then
				break
			end
		end

		if not var_41_24 then
			return false
		end
	end

	return true
end

function var_0_0.ToVShip(arg_42_0, arg_42_1)
	if not arg_42_0.vship then
		arg_42_0.vship = {}

		function arg_42_0.vship.getNation()
			return arg_42_0.vship.config.nationality
		end

		function arg_42_0.vship.getShipType()
			return arg_42_0.vship.config.type
		end

		function arg_42_0.vship.getTeamType()
			return TeamType.GetTeamFromShipType(arg_42_0.vship.config.type)
		end

		function arg_42_0.vship.getRarity()
			return arg_42_0.vship.config.rarity
		end
	end

	arg_42_0.vship.config = arg_42_1

	return arg_42_0.vship
end

function var_0_0.GetShopTypeIdBySkinId(arg_47_0, arg_47_1)
	local var_47_0 = pg.ship_skin_template.get_id_list_by_shop_type_id

	if not arg_47_0.shopTypeIdList then
		arg_47_0.shopTypeIdList = {}
	end

	if arg_47_0.shopTypeIdList[arg_47_1] then
		return arg_47_0.shopTypeIdList[arg_47_1]
	end

	for iter_47_0, iter_47_1 in pairs(var_47_0) do
		for iter_47_2, iter_47_3 in ipairs(iter_47_1) do
			arg_47_0.shopTypeIdList[iter_47_3] = iter_47_0

			if iter_47_3 == arg_47_1 then
				return iter_47_0
			end
		end
	end
end

function var_0_0.willExit(arg_48_0)
	for iter_48_0, iter_48_1 in ipairs(arg_48_0.cards) do
		iter_48_1:Dispose()
	end

	arg_48_0.cards = nil

	pg.UIMgr.GetInstance():UnOverlayPanel(arg_48_0._tf)
end

function var_0_0.onBackPressed(arg_49_0)
	var_0_0.super.onBackPressed(arg_49_0)
end

return var_0_0
