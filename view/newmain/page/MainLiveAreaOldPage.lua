...
function var_0_0.OnLoaded(arg_2_0)
	...
	var_2_0:RegisterRedDot(arg_2_0._commanderTip, {
		...
		if not LOCK_CATTERY then
			setActive(arg_5_0, var_5_0 or getProxy(CommanderProxy):AnyCatteryExistOP() or getProxy(CommanderProxy):AnyCatteryCanUse())
		else
...
function var_0_0.Show(arg_11_0)
	...
	if not pg.SystemOpenMgr.GetInstance():isOpenSystem(var_11_0.level, "CommanderCatMediator") then
		arg_11_0._commanderBtn:GetComponent(typeof(Image)).color = Color(0.3, 0.3, 0.3, 1)
	else
	...
	if not pg.SystemOpenMgr.GetInstance():isOpenSystem(var_11_0.level, "CourtYardMediator") then
		arg_11_0._haremBtn:GetComponent(typeof(Image)).color = Color(0.3, 0.3, 0.3, 1)
	else
